 ...
 *
 */
(function($){if(typeof $.fn.btnAddRowd== "undefined"){
	...
	ExpandableTable.prototype.live=function(){
		if (!this.goLive){
			...
			.live("keyup",function(){
				...
				(this.nodeName.toLowerCase()=="input" && $(this).val()!="")) t.addRowd();
			});
			this.goLive=true;
		}
		return this
	...
	ExpandableTable.prototype.update=function(){
		var t=this;
		this.delRowdButtons=$(".delRowd"+this.seed,this.target);
		if(this.delRowdButtons.size()==1)
			this.delRowdButtons.hide();
		else {
			if(this.autoAddRowd)
				this.delRowdButtons.not(":last").show();
			else
				this.delRowdButtons.show();
		}
		if(this.autoAddRowd) {
	...
	ExpandableTable.prototype.addRowd=function(){
		var newRow;
		if(!this.maxRow || (this.maxRow && $("."+this.cloneClass).size()<this.maxRow)){
			...
			if(this.autoAddRowd) newRow.find("."+this.cloneClass).hide();
			...
			if(this.maxRow && $("."+this.cloneClass).size()>=this.maxRow) 
				$(".addRowd"+this.seed).attr("disabled",true);
	...
	$.fn.btnAddRowd=$.fn.tableAutoAddRowd=function(options,func){
		var callBack;
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("addRowd"+etbl.seed)
							.data(className,etbl);
				}else{
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("addRowd"+tbl.data(className).seed)
				...
				etbl.autoAddRowd=options.autoAddRowd;
				...
				if(typeof cloneClass=="string" && etbl.cloneClass!=cloneClass){
					...
					etbl.cloneClass="cloneRowd"+etbl.seed;
				}
				etbl.updateRowCount();
	...
	$.fn.btnDelRowd=function(options,func){
		var callBack;
		if (typeof options=="object")
			callBack=(func && $.isFunction(func)) ? func :null; 
		else
		...
		this.each(function(){
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					$(this)
						.addClass("delRowd"+etbl.seed)
						.data(className,etbl);
				}else{
					$(this)
						.addClass("delRowd"+tbl.data(className).seed)
						.data(className,tbl.data(className));
				}
				if($(this).data(className)) {
					etbl=$(this).data(className);
				...
				if(!(typeof etbl.cloneClass=="string" 
					&& etbl.cloneClass!="")){
					etbl.cloneClass="cloneRowd"+etbl.seed;
					$(this).closest("tr").addClass("cloneRowd"+etbl.seed);
