...
function getRadioValue(obj) {
	...
	for (i=0; i < obj.length; i++) {
		...
		if (obj[i].checked) {
			...
			return replacehash(obj[i].value);
...
function getdata(frm){
	...
	for(var i=0; i<frm.elements.length;i++){
		...
		if(frm.elements[i].name!='' && frm.elements[i].name!='undefined'){
			...
			if($.inArray(frm.elements[i].name,fields)==-1){

				fields.push(frm.elements[i].name);

				if(frm.elements[i].type=='checkbox'){

					str += '&'+frm.elements[i].name+'=' + getCheckValue(frm.elements[i]);

				}else if(frm.elements[i].type=='radio'){

					str += '&'+frm.elements[i].name+'=' + getRadioValue(frm.elements[i]);

				}else if(frm.elements[i].length==0){

					var s = '';

					for(var j=0;j<frm.elements[i].length;j++){

						s += replacehash(frm.elements[i][j].value)+",";

					}

					s=s.substring(0,s.length-1);

					str += '&'+frm.elements[i].name+'='+s;

				}else{

					str += '&'+frm.elements[i].name+'='+replacehash(frm.elements[i].value);
...
function senddata(page,str,recId){
	...
	$.post("ajax/"+page+".php?rnd="+Math.random(), str,
