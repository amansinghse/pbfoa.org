...
$data = $obj->SelectQuery($query);		
			if($data = $obj->SelectQuery($query)){
				...
				if($pages=$obj->SelectQuery("select f.form_id,f.form_name from admingroup_permissions as p inner join adminuser_forms as f on p.form_id=f.form_id and p.group_id='".$_SESSION['GROUP_ID'] ."' where f.parent_id=0 order by f.display_order")){

					foreach($pages as $page){

						if($data=$obj->SelectQuery("select f.form_code,f.form_name from adminuser_forms as f inner join admingroup_permissions as p on f.form_id=p.form_id and p.group_id='".$_SESSION['GROUP_ID'] ."' where f.parent_id='".$page['form_id']."' order by f.display_order")){

							foreach($data as $row){

								$_SESSION['PAGE_NAME'][$page['form_name']][$row['form_code']]=$row['form_name'];
...
//echo $obj->Encrypt("admin");

//echo $obj->Encrypt("wsbfytw=");
...
.topbar h3{ text-align:left; font-size:15pt; color:#0f4c39;}
...
.header{background:#000; width:100%; float:left; color:#0B5FAE; border-bottom:1px solid #0B5FAE;}
...
<div class="header">
    	...
    	<div class="cols40"><h1  style="color:#fff !important;"><?php echo COMPANY_NAME;?></h1></div>
