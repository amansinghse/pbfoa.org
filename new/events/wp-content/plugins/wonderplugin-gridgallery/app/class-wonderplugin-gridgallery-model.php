...
class WonderPlugin_Gridgallery_Model {
	...
	function generate_body_code($id, $has_wrapper) {
		...
		if ($item_row != null)
		{
			...
			$boolOptions = array('random', 'shownavigation', 'hoverzoomin', 'hoverzoominimageonly', 'hoverzoominimagecenter', 'hoverfade', 'responsive', 'mediumscreen', 'smallscreen', 'showtitle', 'showtexttitle', 'showtextdescription', 'showtextbutton', 'overlaylink','donotaddtext', 'lightboxnogroup', 
					'lightboxshowtitle', 'lightboxshowdescription', 'lightboxresponsive', 'circularimage', 'firstimage', 'textinsidespace', 'donotinit', 'addinitscript');
			...
			if (isset($data->slides) && count($data->slides) > 0)
			{
				...
				if (isset($templates) && count($templates) > 0 && count($templates[0]) > 0)
				{
					...
					{		
						...
						$boolOptions = array('lightbox', 'displaythumbnail', 'lightboxsize', 'weblinklightbox', 'buttonlightbox');
						foreach ( $boolOptions as $key )
						{
						...
						{
							$code_template .= $this->generate_lightbox_code($id, $data, $slide);
						}
						...
						if ( ((isset($slide->lightbox) && $slide->lightbox) || (!empty($slide->weblink))) && ( !isset($data->donotaddtext) || strtolower($data->donotaddtext) === 'false'))
						{
						...
						if ((isset($slide->lightbox) && $slide->lightbox) || (!empty($slide->weblink)))
							$code_template .= '>';
