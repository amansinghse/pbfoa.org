...
$q="INSERT INTO `ad_posts` (`ad_id` ,`ad_user` ,`ad_title` ,`ad_content` ,`ad_image` ,`ad_city` ,`ad_state` ,`ad_zipcode` ,`ad_country` ,`ad_type` ,`ad_time` ,`ad_status`)VALUES (NULL , '$ad_user', '$ad_title', '$ad_content', '$ad_image', '$ad_city', '$ad_state', '$ad_zipcode', '$ad_country', '$ad_type', '$ad_time', '$ad_status');";
...
else if(document.createPage.page_menu_title.value=="")
{
alert("Provide Page Menu Item Title");
document.createPage.page_menu_title.focus();
return false;
}
else if(document.createPage.page_heading.value=="")
{
alert("Provide Page Heading");
return false;
}
else
{
...
?>
								<tr>
									...
									<td align="left" valign="top">
										<!-- Icons -->
										<a href="admin.php?mod=editPost&aid=<?php echo $qrow['ad_id'].'&token='.time(); ?>" title="Edit"><img src="resources/images/icons/pencil.png" alt="Edit" /></a>
...
<form name="createPost" action="admin.php?mod=managePosts&token=<?php echo time(); ?>" method="post" onsubmit="return validate();" enctype="multipart/form-data">
...
<?php
$q="SELECT * FROM `ad_cat` WHERE `cat_parent`='1';";
$qr=mysql_query($q);
while($qrow=mysql_fetch_array($qr,MYSQL_ASSOC))
{

?>
			<input type="checkbox" name="ad_cat" value="<?php echo $qrow['cat_id']; ?>" /><?php echo $qrow['cat_name']; ?><br />
...
?>
						...
						<?php
$q="SELECT * FROM `ad_cat` WHERE `cat_parent`='2';";
$qr=mysql_query($q);
while($qrow=mysql_fetch_array($qr,MYSQL_ASSOC))
{
